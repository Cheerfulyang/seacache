...
/*
 * Define default  a send and recv ring for a worker core and write core, but only a one mempool
    is configured for each NUMA node
 */
#define WORKER_2_DISPATCH_RECV_RING_NAME         "WORKER_2_D_RECV_RING_%u_NAME"
#define WORKER_2_TX_SEND_RING_NAME    	         "WORKER_2_TX_SEND_RING_%u_NAME"
#define WRITE_2_WORKER_SEND_RING_NAME    	 "WRITE_2_WOKER_SEND_RING_%u_NAME"
#define WRITE_2_WORKER_RECV_RING_NAME    	 "WRITE_2_WORKER_RECV_RING_%u_NAME"
#define TX_2_WRITE_RECV_RING_NAME     	 	 "TX_2_WRITE_REVE_RING_%u_NAME"
#define TX_2_WORKER_RECV_RING_NAME  	 	 "TX_2_WORKER_RECV_RING_%u_NAME"
...
#define SCHEDULE_MEMPOOL_NAME_FLAG      7
...
struct stats {
	...
	uint32_t int_no_hit;    /**< number of Interest packets that can not be served by CS */

	uint64_t total_cpu_cycle;     /**< count cpu cycle of one metric */
	uint64_t total_number;        /**< total pkt number of test        */

	uint32_t nb_chunk_write_to_ssd;   /**< number of segment that is written to SSD cache */
	uint32_t nb_chunk_read_from_ssd;  /**< number of segment that is read from SSD cache */
