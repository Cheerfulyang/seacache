...
char *  get_rx_queue_name(unsigned id, unsigned flag){
	...
	* by maximum 3 digits (plus an extra byte for safety) */
	static char worker_2_dispatch_recv_ring[sizeof(WORKER_2_DISPATCH_RECV_RING_NAME)+2];
	static char worker_2_tx_send_ring[sizeof(WORKER_2_TX_SEND_RING_NAME)+2];
	static char write_2_worker_send_ring[sizeof(WRITE_2_WORKER_SEND_RING_NAME)+2];
	static char write_2_worker_recv_ring[sizeof(WRITE_2_WORKER_RECV_RING_NAME)+2];
	...
	if (flag == WORKER_2_DISPATCH_RECV_RING_NAME_FLAG){
		snprintf(worker_2_dispatch_recv_ring, sizeof(worker_2_dispatch_recv_ring)-1, WORKER_2_DISPATCH_RECV_RING_NAME, id);
		return worker_2_dispatch_recv_ring;

	}
	else if (flag == WORKER_2_TX_SEND_RING_NAME_FLAG){
		snprintf(worker_2_tx_send_ring, sizeof(worker_2_tx_send_ring)-1, WORKER_2_TX_SEND_RING_NAME, id);
		return worker_2_tx_send_ring;

	}
	else if (flag == WRITE_2_WORKER_SEND_RING_NAME_FLAG){
		snprintf(write_2_worker_send_ring, sizeof(write_2_worker_send_ring)-1, WRITE_2_WORKER_SEND_RING_NAME, id);
		return write_2_worker_send_ring;

	}
	else if (flag == WRITE_2_WORKER_RECV_RING_NAME_FLAG){
		snprintf(write_2_worker_recv_ring, sizeof(write_2_worker_recv_ring)-1, WRITE_2_WORKER_RECV_RING_NAME, id);
		return write_2_worker_recv_ring;
	}
	else if (flag == TX_2_WRITE_RECV_RING_NAME_FLAG)
	{
		snprintf(tx_2_write_recv_ring, sizeof(tx_2_write_recv_ring)-1, TX_2_WRITE_RECV_RING_NAME, id);
		return tx_2_write_recv_ring;
	}
	else if (flag == TX_2_WORKER_RECV_RING_NAME_FLAG)
	{
		snprintf(tx_2_worker_recv_ring, sizeof(tx_2_worker_recv_ring)-1, TX_2_WORKER_RECV_RING_NAME, id);
		return tx_2_worker_recv_ring;
	}
	else if (flag == SCHEDULE_MEMPOOL_NAME_FLAG)
	{
		snprintf(buffer_mempool, sizeof(buffer_mempool)-1, SCHEDULE_MEMPOOL_NAME, id);
...
static int
init_mbuf_pools(struct app_global_config *app, struct app_lcore_params lcore[])
{
	int socket_id;
	unsigned lcore_id;
	char pool_name[64];
		...
		if (pool[socket_id] == NULL) {
			...
			if (pool[socket_id] == NULL) {
				rte_exit(EXIT_FAILURE,
					"Cannot init mbuf pool on socket %d, err=-%d \n", socket_id, rte_errno);
			}
			else {
...
init_ports( uint32_t portmask, uint8_t nb_rx_queues, uint8_t nb_tx_queues) {
	...
	for (port_id = 0; port_id < nb_ports; port_id++) {
		 ...
		 */
		ret = rte_eth_dev_configure(port_id, nb_rx_queues, nb_tx_queues, &port_conf_default);
		if (ret < 0) {
...
*/
static int
init_shm_rings(struct app_lcore_params lcore[])
{
    unsigned flags = 0; 
    unsigned socket_id;
    unsigned lcore_id;
    ...
    for (lcore_id = 0; lcore_id < APP_MAX_LCORES; lcore_id++){
        ...
        if (!rte_lcore_is_enabled(lcore_id)|| lcore_id == MASTER_LCORE
             || lcore_id == DISPATCH_LCORE ) 
             { continue;}
        ...
        if( message_pool[socket_id] == NULL ) {
            message_pool[socket_id] = rte_mempool_create(mempool_name, 
                                                         SCHEDULE_MEMPOOL_SIZE ,          // Number of elements 1024
            ...
            if( message_pool[socket_id] == NULL ){
                rte_exit(EXIT_FAILURE,
                        "Cannot init memory pool used for communication for fwd and ssd core on socket %d\n", 
                         socket_id);                
            }
        }
		...
		{
	       	   core_send_ring_name = get_rx_queue_name(lcore_id, WORKER_2_TX_SEND_RING_NAME_FLAG);
        	   core_recv_ring_name = get_rx_queue_name(lcore_id, WORKER_2_DISPATCH_RECV_RING_NAME_FLAG);
        
        	// flags 0 means that mutiple producer and consumer. One producer and one consumer
        	lcore[lcore_id].send_ring = rte_ring_create(core_send_ring_name, 
                                                      	  RING_CHUNK_NOTIFY_SHEDULE_SIZE,  //The size of the ring 1024
                                                      	  socket_id, 
                                                      	  flags); 
                lcore[lcore_id].recv_ring = rte_ring_create(core_recv_ring_name,
                                                        RING_DISPATCH_2_WORKER_SIZE,  //The size of the dispatch to worker ring 4096
                                                        socket_id, 
                                                        flags);   

        	//Check if we allocate success
       		if (lcore[lcore_id].send_ring == NULL || lcore[lcore_id].recv_ring == NULL) {
                printf("Cannot init ring used for communication for worker and dispatch/TX core for worker lcore_id:%u \n", 
                      	lcore_id);
            	rte_exit(EXIT_FAILURE,
                        "Cannot init ring used for communication for worker and dispatch/TX core for worker lcore_id:%u \n", 
                      	lcore_id);
        	}else{
                INIT_LOG("Allocated ring used for communication for worker and dispatch/TX core for worker lcore_id:%u \n", 
                      lcore_id);
        	}           
		}
		...
		{
		   core_send_ring_name     = get_rx_queue_name(lcore_id, TX_2_WRITE_RECV_RING_NAME_FLAG);
           core_recv_ring_name     = get_rx_queue_name(lcore_id, TX_2_WORKER_RECV_RING_NAME_FLAG);
        	...
        	//Check if we allocate success
       		if (lcore[lcore_id].send_ring == NULL || lcore[lcore_id].recv_ring == NULL) {
            	rte_exit(EXIT_FAILURE,
                        "Cannot init ring used for communication for tx and worker/write core for tx lcore_id:%u \n", 
                      	lcore_id);
        	}else {
                INIT_LOG("Allocated ring used for communication for tx and worker/write core for tx lcore_id:%u \n", 
                      lcore_id);
        	}      
		}
		...
		{
		   core_send_ring_name     = get_rx_queue_name(lcore_id, WRITE_2_WORKER_SEND_RING_NAME_FLAG );
           core_recv_ring_name     = get_rx_queue_name(lcore_id, WRITE_2_WORKER_RECV_RING_NAME_FLAG );
        	...
        	//Check if we allocate success
       		if (lcore[lcore_id].send_ring == NULL || lcore[lcore_id].recv_ring == NULL) {
            	rte_exit(EXIT_FAILURE,
                        "Cannot init ring used for communication for write and worker core for write lcore_id:%u \n", 
                      	lcore_id);
        	}else {
                INIT_LOG("Allocated ring used for communication for write and worker core for write lcore_id:%u \n", 
                      lcore_id);
        	}   
		}   
   }
   //If init failed, this function will exit directly. So the return value may not have meaning 
   return 0;
}
...
init_queues(uint32_t portmask, struct app_lcore_params lcore[]) {
	...
	for (lcore_id = 0; lcore_id < APP_MAX_LCORES; lcore_id++) {
		...
		if ((!rte_lcore_is_enabled(lcore_id))||(lcore_id == MASTER_LCORE)
                || (lcore_id > 13 && lcore_id < 24)||(lcore_id > 1 && lcore_id < 12 ) ) { //10,11,22,23 are free
			continue;
		}
...
static void init_storage_data_structures(struct app_lcore_params lcore[]) {
	...
	for (lcore_id = 0; lcore_id < APP_MAX_LCORES; lcore_id++) {
		...
		if ((!rte_lcore_is_enabled(lcore_id)) || lcore_id == MASTER_LCORE
			    || lcore_id == DISPATCH_LCORE || lcore_id == TX_LCORE_FIRST
			    || lcore_id == TX_LCORE_SECOND||  (lcore_id > 13 && lcore_id < 24)
			    || lcore_id == 10 || lcore_id == 11) 
			{ continue; }
		socket_id = rte_lcore_to_socket_id(lcore_id);
      
        //  70.8W, 1280
  		lcore[lcore_id].cs_two = cs_two_create(BUCKET_NUM_OF_HASH_TABLE,
			                     DRAM_LRU_QUEUE_SIZE_PER_CORE, socket_id);
...
void 
init_app(struct app_global_config *app, struct app_lcore_params lcore[]) {

	//uint8_t nb_lcores = get_nb_lcores_available();

	//One core is reserved for control plane, two cores are reserved for SSD IO operation
	//nb_lcores -= 3; 
    ...
    init_shm_rings(lcore);

     /*Only fwd core, excluding SSD IO core and control core, owns this data structure */
	INIT_LOG("Initializing SEANET forwarding data structures (CS_TWO)\n");
	init_storage_data_structures(lcore);

	/*
	The nb_lcores will be used to configure the queue number of a port. Notice this value must 
	set correct
	*/
	INIT_LOG("Initializing ports\n");
	init_ports(app->portmask,1,2);

	/*Only fwd core, excluding SSD IO core and control core, will be binded with a queue of a port */
