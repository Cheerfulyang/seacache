 ...
 */
int set_bitmap_from_offset(int set_num, uint32_t offset, struct Bitmap_info *bitmap_info)
{
    int packet_num = offset / PACKET_PAYLOAD_LEN;
    if (bitmap_info == NULL)
        return -1;
    if (packet_num < 0 || packet_num >= bitmap_info->packet_num_of_one_chunk || bitmap_info->bitmap == NULL)
        return -1;
    if (set_num == 1)
    {
        bitmap_info->bitmap[packet_num >> BITMAP_SHIFT] |= (1 << (packet_num & BITMAP_MASK));
    }
    else if (set_num == 0)
    {
        bitmap_info->bitmap[packet_num >> BITMAP_SHIFT] &= ~(1 << (packet_num & BITMAP_MASK));
...
/* 查询位操作用&操作符 */
int check_bitmap_from_offset(uint32_t offset, struct Bitmap_info *bitmap_info)
{
    if (bitmap_info == NULL)
        return -1;
    int packet_num = offset / PACKET_PAYLOAD_LEN;
    if (packet_num < 0 || packet_num >= bitmap_info->packet_num_of_one_chunk || bitmap_info->bitmap == NULL)
        return -1;
    return (bitmap_info->bitmap[packet_num >> BITMAP_SHIFT] & (1 << (packet_num & BITMAP_MASK))) >> (packet_num & BITMAP_MASK);
}
...
int bitmap_is_full(struct Bitmap_info *bitmap_info)
{
    ...
    if (bitmap_info == NULL)
        return -1;
    if (bitmap_info->bitmap == NULL || bitmap_info-> packet_num_of_one_chunk== 0)
        return -1;
    quotient = bitmap_info->packet_num_of_one_chunk / BITMAP_WORD;
    remain = bitmap_info->packet_num_of_one_chunk % BITMAP_WORD;
    //printf("%d %d %d  %d\n", BITMAP_WORD, bitmap_info->packet_num_of_one_chunk, quotient, remain);
    ...
    for (i = 0; i < remain; i++)
    {
        if (check_bitmap_from_offset(bitmap_info->packet_num_of_one_chunk - 1 - i, bitmap_info) == 0)
            return 0;
    }
    return 1;
}
