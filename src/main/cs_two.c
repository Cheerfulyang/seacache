...
static inline void push_chunk_to_other_core(struct rte_mempool *shm_message_pool, struct rte_ring *shm_ring_queue,
...
*/
static inline int lookup_cache(cs_two_t *cs, uint32_t bucket, char *eid, uint32_t offset)
{
...
static inline int8_t __cs_two_insert_with_hash(struct rte_mempool *shm_message_pool, struct rte_ring *shm_ring_queue,
...
static inline uint8_t __cs_two_lookup_with_hash(struct rte_mempool *shm_message_pool,
    ...
    while (1)
    {
        ...
        for (tab = 0; tab < ENTRY_NUM_OF_ONE_BUCKET; tab++)
        {
            ...
            if (a_chunk_msg_desc_to_tx != NULL)
            {
                ...
                a_chunk_msg_desc_to_tx->chunk_size = SIZE_OF_ONE_CHUNK; //offst and length should be considered further
                rte_memcpy(a_chunk_msg_desc_to_tx->chunk, cs->dram_queue[dram_queue_index].dram_packet_pool_chunk_addr, SIZE_OF_ONE_CHUNK);
                rte_memcpy(&a_chunk_msg_desc_to_tx->mbuf, mbuf, sizeof(struct rte_mbuf));
            }
